...
MLCDynDirectStream::MLCDynDirectStream(
    ...
    : MLCDynStream(_configData, _controller, _responseMsgBuffer,
      ...
      slicedStream(_configData,
                   _controller->getRubySystem()->getBlockSizeBytes()),
...
void MLCDynDirectStream::sendCreditToLLC(const LLCSegmentPosition &segment) {
  ...
  MLC_S_DPRINTF_(MLCRubyStreamLife, this->strandId,
                 "Extended %lu (Elem %lu) -> %lu at %s (NoMigration %d).\n",
  ...
  auto msg = std::make_shared<ruby_stream::RequestMsg>(
      ...
      this->rubySystem);
  msg->m_addr = this->rubySystem->makeLineAddress(remotePAddr);
  msg->m_Type = ruby_stream::CoherenceRequestType_STREAM_FLOW;
...
void MLCDynDirectStream::notifyIndStreams(const MLCStreamSlice &slice) {
  ...
  for (auto elemIdx = sliceId.getStartIdx(); elemIdx < sliceId.getEndIdx();
    ...
    auto elementLineOffset =
        elementVAddr % this->rubySystem->getBlockSizeBytes();
     ...
     */
    if (this->rubySystem->makeLineAddress(elementVAddr) !=
...
void MLCDynDirectStream::sendCommitToLLC(const LLCSegmentPosition &segment) {
  ...
  auto startPAddrLine = this->rubySystem->makeLineAddress(segment.startPAddr);
  ...
  msg->m_Type = ruby_stream::CoherenceRequestType_STREAM_COMMIT;
  msg->m_Requestors.add(this->controller->getMachineID());
  msg->m_Destination.add(remoteBank);
  msg->m_MessageSize = ruby::MessageSizeType_Control;
  DynStreamSliceId sliceId;
