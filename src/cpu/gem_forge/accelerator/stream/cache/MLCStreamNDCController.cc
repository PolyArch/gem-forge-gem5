...
MLCStreamNDCController::MLCStreamNDCController(MLCStreamEngine *_mlcSE)
    : rubySystem(_mlcSE->getRubySystem()), mlcSE(_mlcSE) {}
...
void MLCStreamNDCController::receiveStreamNDCRequest(PacketPtr pkt) {
  ...
  for (auto streamNDC : *streamNDCs) {
    ...
    RequestPtr req = std::make_shared<Request>(
        paddrLine, streamNDC->stream->getMemElementSize(), 0,
        pkt->requestorId());
    ...
    auto msg = std::make_shared<ruby_stream::RequestMsg>(
        mlcSE->controller->clockEdge(), this->rubySystem->getBlockSizeBytes(),
        this->rubySystem);
    msg->m_addr = paddrLine;
    msg->m_Type = ruby_stream::CoherenceRequestType_STREAM_NDC;
...
void MLCStreamNDCController::receiveStreamNDCResponse(
