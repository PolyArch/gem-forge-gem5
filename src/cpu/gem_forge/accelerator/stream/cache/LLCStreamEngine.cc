...
LLCStreamEngine::LLCStreamEngine(
    ...
    : ruby::Consumer(_controller), rubySystem(_controller->getRubySystem()),
      controller(_controller), streamMigrateMsgBuffer(_streamMigrateMsgBuffer),
...
void LLCStreamEngine::receiveStreamMigrate(LLCDynStreamPtr dynS,
  ...
  [[maybe_unused]] Addr paddrLine = this->rubySystem->makeLineAddress(paddr);
...
void LLCStreamEngine::receiveStreamData(
  ...
  if (dynS->configData->hasDepRemoteNestRegion) {
    ...
    if (!S->isLoadComputeStream()) {
      ...
      auto lineOffset = sliceId.vaddr % this->rubySystem->getBlockSizeBytes();
...
void LLCStreamEngine::receiveStoreStreamData(
  ...
  auto sliceLineVAddr = this->rubySystem->makeLineAddress(sliceId.vaddr);
  auto sliceLinePAddr = this->rubySystem->makeLineAddress(paddr);
  auto blockSize = this->rubySystem->getBlockSizeBytes();
  for (auto idx = sliceId.getStartIdx(); idx < sliceId.getEndIdx(); ++idx) {
...
void LLCStreamEngine::generateMulticastRequest(RequestQueueIter reqIter,
  ...
  for (auto idx = 0; idx < group.size(); ++idx) {
    ...
    if (reqType == ruby_stream::CoherenceRequestType_GETU) {
...
void LLCStreamEngine::issueStreamDirect(LLCDynStream *dynS) {
  ...
  if (dynS->translateToPAddr(vaddr, paddr)) {
    ...
    Addr vaddrLine = this->rubySystem->makeLineAddress(vaddr);
    Addr paddrLine = this->rubySystem->makeLineAddress(paddr);
...
LLCStreamEngine::getStreamReqType(LLCDynStream *stream) const {
  ...
  case ::LLVM::TDG::StreamInfo_Type_LD: {
    if (SS->isUpdateStream()) {
      ...
      reqType = ruby_stream::CoherenceRequestType_GETH;
      ...
      if (auto dynS = stream->getCoreDynS()) {
        if (dynS->shouldCoreSEIssue()) {
          ...
          reqType = ruby_stream::CoherenceRequestType_GETU;
        }
      } else {
...
void LLCStreamEngine::issueIndirectLoadReq(LLCDynStream *dynIS,
  ...
  const auto blockBytes = this->rubySystem->getBlockSizeBytes();
  ...
  LLC_SLICE_DPRINTF(sliceId,
                    ...
                    dynIS->inflyRequests,
                    ruby_stream::CoherenceRequestType_to_string(reqType),
                    elemVAddr, dynCoreIS != nullptr,
  ...
  while (totalSliceSize < elemSize) {
    ...
    if (dynIS->translateToPAddr(curSliceVAddr, curSlicePAddr)) {
      Addr curSliceVAddrLine = this->rubySystem->makeLineAddress(curSliceVAddr);
      Addr curSlicePAddrLine = this->rubySystem->makeLineAddress(curSlicePAddr);
...
void LLCStreamEngine::issueIndirectStoreOrAtomicReq(LLCDynStream *dynIS,
  ...
  if (dynIS->translateToPAddr(elemVAddr, elemPAddr)) {
    this->incrementIssueSlice(IS->statistic);
    auto vaddrLine = this->rubySystem->makeLineAddress(elemVAddr);
    auto paddrLine = this->rubySystem->makeLineAddress(elemPAddr);
    /**
    ...
    if (this->controller->isStreamIdeaStoreEnabled()) {
      ...
      if (dynIS->prevStorePAddrLine == paddrLine) {
        ...
        if (IS->isDirectMemStream()) {
          storeSize = this->rubySystem->getBlockSizeBytes();
        }
      }
    }
...
void LLCStreamEngine::issueIndirectAtomicUnlockRequest(
  ...
  if (dynIS->translateToPAddr(elemVAddr, elemPAddr)) {
     ...
     * Push to the request queue.
     */
    auto reqType = ruby_stream::CoherenceRequestType_STREAM_UNLOCK;
...
LLCStreamEngine::RequestQueueIter LLCStreamEngine::enqueueRequest(
    ...
    ruby::MachineType destMachineType, ruby_stream::CoherenceRequestType type) {
  ...
  LLC_SLICE_DPRINTF(sliceId, "Enqueue %s Req: Start Translation.\n",
                    ruby_stream::CoherenceRequestType_to_string(type));
...
void LLCStreamEngine::translationCallback(PacketPtr pkt, ThreadContext *tc,
  ...
  LLC_SLICE_DPRINTF(
      reqIter->sliceId, "Translated %s Req.\n",
      ruby_stream::CoherenceRequestType_to_string(reqIter->requestType));
...
void LLCStreamEngine::issueStreamReqToRemoteBank(const LLCStreamRequest &req) {
  ...
  if (req.requestType == ruby_stream::CoherenceRequestType_STREAM_STORE) {
  ...
  if (handledHere) {
     ...
     */
    if (req.requestType == ruby_stream::CoherenceRequestType_GETH) {
  ...
  if (req.requestType == ruby_stream::CoherenceRequestType_STREAM_FORWARD ||
      req.requestType == ruby_stream::CoherenceRequestType_STREAM_STORE) {
    /**
    ...
    if (dynS) {
      auto totalNodesBeforeLLC =
          this->rubySystem->MachineType_base_number(ruby::MachineType_L2Cache);
      auto selfLLCNodeId =
          this->rubySystem->getRawNodeID(selfMachineId) - totalNodesBeforeLLC;
      auto destLLCNodeId =
          this->rubySystem->getRawNodeID(destMachineId) - totalNodesBeforeLLC;
      if (req.requestType == ruby_stream::CoherenceRequestType_STREAM_FORWARD) {
        dynS->getStaticS()->statistic.sampleLLCSendTo(selfLLCNodeId,
                                                      destLLCNodeId);
      } else if (req.requestType ==
                 ruby_stream::CoherenceRequestType_STREAM_STORE) {
        if (dynS->isIndirect()) {
          dynS->getStaticS()->statistic.sampleIndReq(selfLLCNodeId,
                                                     destLLCNodeId);
        }
      }
    }
  ...
  if (handledHere) {
     ...
     */
    if (req.requestType == ruby_stream::CoherenceRequestType_STREAM_STORE ||
        req.requestType == ruby_stream::CoherenceRequestType_GETH ||
        req.requestType == ruby_stream::CoherenceRequestType_GETU) {
...
LLCStreamEngine::ResponseMsgPtr LLCStreamEngine::createStreamMsgToMLC(
    const DynStreamSliceId &sliceId, ruby_stream::CoherenceResponseType type,
  ...
  // Try to copy data.
  if (data) {
    assert(lineOffset + dataSize <= this->rubySystem->getBlockSizeBytes());
...
void LLCStreamEngine::issueStreamMsgToMLC(ResponseMsgPtr msg, bool forceIdea) {
  ...
  if (forceIdea) {
    ...
    LLC_SLICE_DPRINTF(
        sliceId, "Send ideal %s to MLC.\n",
        ruby_stream::CoherenceResponseType_to_string(msg->m_Type));
  } else {
    /**
...
void LLCStreamEngine::issueStreamAckToMLC(const DynStreamSliceId &sliceId,
  ...
  auto msg = this->createStreamMsgToMLC(
      sliceId, ruby_stream::CoherenceResponseType_STREAM_ACK, paddrLine,
      nullptr, 0, 0, 0);
...
void LLCStreamEngine::issueStreamDoneToMLC(const DynStreamSliceId &sliceId,
  ...
  auto paddrLine = 0;
  auto msg = this->createStreamMsgToMLC(
      sliceId, ruby_stream::CoherenceResponseType_STREAM_DONE, paddrLine,
      nullptr, 0, 0, 0);
  this->issueStreamMsgToMLC(msg, forceIdea);
}
...
void LLCStreamEngine::issueStreamRangeToMLC(DynStreamAddressRangePtr &range,
  ...
  auto msg = this->createStreamMsgToMLC(
      sliceId, ruby_stream::CoherenceResponseType_STREAM_RANGE, paddrLine,
      nullptr, 0, 0, 0);
  msg->m_range = range;
  this->issueStreamMsgToMLC(msg, forceIdea);
}
...
void LLCStreamEngine::issueStreamDataToMLC(const DynStreamSliceId &sliceId,
  ...
  auto msg = this->createStreamMsgToMLC(
      sliceId, ruby_stream::CoherenceResponseType_DATA_EXCLUSIVE, paddrLine,
      data, dataSize, payloadSize, lineOffset);
  this->issueStreamMsgToMLC(msg, forceIdea);
}
...
void LLCStreamEngine::issueStreamDataToLLC(
  ...
  for (const auto &sendToEdge : sendToEdges) {
    ...
    auto recvElemVAddrEndLine =
        this->rubySystem->makeLineAddress(recvElemVAddr);
    ...
    if (dynS->translateToPAddr(recvElemVAddrLine, recvElemPAddrLine)) {
      ...
      if (auto recvDynS = LLCDynStream::getLLCStream(recvStrandId)) {
        if (recvDynS->isMigrationDisabled()) {
          recvElemPAddrLine = this->rubySystem->makeLineAddress(
...
void LLCStreamEngine::issueNonMigrateStreamDataToLLC(
  ...
  auto recvElemVAddrLine = this->rubySystem->makeLineAddress(sliceId.vaddr);
  auto recvElemPAddrLine = this->rubySystem->makeLineAddress(
      dynS->getLLCController()->getAddressToOurLLC());

  auto reqIter = this->enqueueRequest(
      dynS->getStaticS(), sliceId, recvElemVAddrLine, recvElemPAddrLine,
      recvElemMachineType, ruby_stream::CoherenceRequestType_STREAM_FORWARD);
...
void LLCStreamEngine::issuePUMPrefetchStreamDataToLLC(
  ...
  auto paddrLine = this->rubySystem->makeLineAddress(paddr);
  ...
  this->pumEngine->sendPUMDataToLLC(
      sliceId, recvBanks, this->rubySystem->getBlockSizeBytes(), isPUMPrefetch);
  for (const auto &dstNodeId : recvBanks.getAllDest()) {
...
void LLCStreamEngine::migrateStreams() {
  ...
  while (streamIter != streamEnd && migrated < this->migrateWidth &&
    ...
    auto nextMachineId = this->controller->mapAddressToLLCOrMem(
        this->rubySystem->makeLineAddress(nextPAddr), nextMachineType);
...
void LLCStreamEngine::migrateStream(LLCDynStream *stream) {
  ...
  auto msg = std::make_shared<ruby_stream::StreamMigrateRequestMsg>(
      this->controller->clockEdge(), this->rubySystem->getBlockSizeBytes(),
      this->rubySystem);
  msg->m_addr = paddrLine;
  msg->m_Type = ruby_stream::CoherenceRequestType_STREAM_MIGRATE;
...
void LLCStreamEngine::receiveStreamIndirectReq(const RequestMsg &req) {
...
void LLCStreamEngine::receiveStreamIndirectReqImpl(const RequestMsg &req) {
  ...
  LLC_SLICE_DPRINTF(sliceId, "Recv [ind] %s req paddrLine %#x NoC delay %s.\n",
                    ruby_stream::CoherenceRequestType_to_string(req.m_Type),
                    req.m_addr, networkLatency);

  if (req.m_Type == ruby_stream::CoherenceRequestType_STREAM_FORWARD) {
  ...
  auto msg = std::make_shared<ruby_stream::RequestMsg>(req);
  Cycles latency(1);
...
void LLCStreamEngine::processStreamFwdReq(const RequestMsg &req,
  ...
  if (sliceId.getDynStreamId() != dynS->getDynStreamId()) {
    ...
    if (dynS->isMigrationDisabled()) {
      ...
      auto vaddrLine = this->rubySystem->makeLineAddress(sliceId.vaddr);
      Addr paddrLine;
...
bool LLCStreamEngine::tryToProcessIndirectAtomicUnlockReq(
  ...
  if (req.m_Type != ruby_stream::CoherenceRequestType_STREAM_UNLOCK) {
...
void LLCStreamEngine::triggerUpdate(LLCDynStreamPtr dynS,
   ...
   */
  Addr loadBlockVAddrLine = this->rubySystem->makeLineAddress(sliceId.vaddr);
  int elementOffset = 0;
...
void LLCStreamEngine::triggerAtomic(LLCDynStreamPtr dynS,
  ...
  auto lineOffset = elemVAddr % this->rubySystem->getBlockSizeBytes();
  if (lineOffset + elemMemSize > this->rubySystem->getBlockSizeBytes()) {
  ...
  if (loadValueBlock) {
    ...
    if (lineOffset + elemCoreSize > this->rubySystem->getBlockSizeBytes()) {
...
void LLCStreamEngine::processLoadComputeSlice(LLCDynStreamPtr dynS,
  ...
  auto sliceVAddrLine = this->rubySystem->makeLineAddress(sliceId.vaddr);
  Addr paddrLine;
  ...
  for (auto elemIdx = sliceId.getStartIdx(); elemIdx < sliceId.getEndIdx();
    ...
    int overlapSize = elem->computeLoadComputeOverlap(
        sliceVAddrLine, this->rubySystem->getBlockSizeBytes(), sliceOffset,
        elemOffset);
    if (overlapSize == 0) {
      continue;
    }
  ...
  if (payloadSize > this->rubySystem->getBlockSizeBytes()) {
    payloadSize = this->rubySystem->getBlockSizeBytes();
  }
  ...
  if (coreNeedValue) {
    ...
    this->issueStreamDataToMLC(
        sliceId, paddrLine,
        loadValueBlock.getData(0, this->rubySystem->getBlockSizeBytes()),
        this->rubySystem->getBlockSizeBytes(), payloadSize /* payloadSize */,
...
void LLCStreamEngine::processDirectAtomicSlice(
  ...
  if (coreNeedValue) {
    ...
    this->issueStreamDataToMLC(
        ...
        loadValueBlock->getData(0, this->rubySystem->getBlockSizeBytes()),
        this->rubySystem->getBlockSizeBytes(),
        std::min(totalPayloadSize, this->rubySystem->getBlockSizeBytes()),
        0 /* Line offset */);
    LLC_SLICE_DPRINTF(sliceId,
...
void LLCStreamEngine::postProcessDirectUpdateSlice(
  ...
  if (coreNeedValue) {
    ...
    for (auto idx = sliceId.getStartIdx(); idx < sliceId.getEndIdx(); ++idx) {
       ...
       */
      const auto lineSize = this->rubySystem->getBlockSizeBytes();
      Addr loadBlockVAddrLine =
          this->rubySystem->makeLineAddress(sliceId.vaddr);
      int elementOffset = 0;
      int loadBlockOffset = 0;
...
void LLCStreamEngine::performStore(Addr paddr, int size, const uint8_t *value) {
  ...
  assert((paddr % this->rubySystem->getBlockSizeBytes()) + size <=
             this->rubySystem->getBlockSizeBytes() &&
...
LLCStreamEngine::performStreamAtomicOp(LLCDynStreamPtr dynS,
  ...
  assert((elemPAddr % this->rubySystem->getBlockSizeBytes()) + elemSize <=
             this->rubySystem->getBlockSizeBytes() &&
...
void LLCStreamEngine::tryVectorizeElem(LLCStreamElementPtr &elem,
  ...
  if (elem->S->isReduction()) {
     ...
     */
    auto elemsPerLine = this->rubySystem->getBlockSizeBytes() / elem->size;
