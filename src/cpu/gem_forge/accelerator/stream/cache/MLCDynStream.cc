...
MLCDynStream::MLCDynStream(CacheStreamConfigureDataPtr _configData,
    ...
    : stream(_configData->stream),
      ...
      rubySystem(_controller->getRubySystem()), controller(_controller),
...
void MLCDynStream::recvCoreReq(const DynStreamSliceId &sliceId) {
  ...
  if (slice->dataReady) {
    ...
    if (slice->coreSliceId.vaddr !=
        this->rubySystem->makeLineAddress(slice->sliceId.vaddr)) {
...
void MLCDynStream::makeResponse(MLCStreamSlice &slice) {
  ...
  auto upperMachineId =
      ruby::MachineID(static_cast<ruby::MachineType>(selfMachineId.type - 1),
                      selfMachineId.num);
  auto msg = std::make_shared<ruby_stream::CoherenceMsg>(
      this->controller->clockEdge(), this->rubySystem->getBlockSizeBytes(),
      this->rubySystem);
  msg->m_addr = paddrLine;
  msg->m_Class = ruby_stream::CoherenceClass_DATA_EXCLUSIVE;
  ...
  if (this->stream->isAtomicComputeStream() ||
    ...
    msg->m_Class = ruby_stream::CoherenceClass_STREAM_FROM_MLC;
  }
  ...
  if (debug::DEBUG_TYPE) {
    std::stringstream ss;
    auto lineOffset =
        slice.sliceId.vaddr % this->rubySystem->getBlockSizeBytes();
